...
//Rare exceptions to indicate a bug in the _shortstop code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_shortstop = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_shortstop]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.shortstop.module"
